...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            next_qs = torch.stack([next_qs[self.num_critic_networks - 1 - i] for i in range(self.num_critic_networks)])
        elif self.target_critic_backup_type == "min":
            min = torch.min(next_qs, 0)
            next_qs = torch.stack([min for _ in range(self.num_critic_networks)])
        elif self.target_critic_backup_type == "mean":
            mean = torch.mean(next_qs, 0)
            next_qs = torch.stack([mean for _ in range(self.num_critic_networks)])
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + (~done) * next_qs * self.discount
        ...
        # Compute loss
        loss: torch.Tensor = nn.MSELoss()(q_values, target_values)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # entropy = - action_distribution.log_prob(action_distribution.sample()).mean()
        # if self.actor_gradient_type == "reparametrize":
        #    entropy = - action_distribution.log_prob(action_distribution.rsample()).mean()
        return - action_distribution.log_prob(action_distribution.rsample()).mean()
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = self.critic(obs=torch.stack([obs for _ in range(self.num_actor_samples)], dim=0), action=action)
